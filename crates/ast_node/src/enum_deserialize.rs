      ...
      for variant in &data.variants {
        ...
        let (str_pat, bytes_pat) = {
            ...
            }
          {
            ...
            if tags.len() == 1 {
              make_pat(tags.first().unwrap().clone().into())
            } else {
              let mut str_cases = Punctuated::new() as Punctuated<Pat, _>;
              let mut bytes_cases = Punctuated::new() as Punctuated<Pat, _>;
              for tag in tags {
                let (str_pat, bytes_pat) = make_pat((tag as Lit).clone());
